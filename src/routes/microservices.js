...
module.exports = [
  ...
  {
    method: 'patch',
    path: '/api/v3/microservices/:uuid/models',
    supportSubstitution: true,
    middleware: async (req, res) => {
      logger.apiReq(req)
      ...
      await rbacMiddleware.protect()(req, res, async () => {
        const updateMicroserviceCatalogEndPoint = ResponseDecorator.handleErrors(MicroservicesController.updateMicroserviceCatalogEndPoint,
          successCode, errorCodes)
        const responseObject = await updateMicroserviceCatalogEndPoint(req)
        const user = req.kauth && req.kauth.grant && req.kauth.grant.access_token ? req.kauth.grant.access_token.content.preferred_username : 'system'
        res
          .status(responseObject.code)
          .send(responseObject.body)
  ...
  {
    method: 'patch',
    path: '/api/v3/microservices/:uuid/knowledge',
    supportSubstitution: true,
    middleware: async (req, res) => {
      logger.apiReq(req)
      ...
      await rbacMiddleware.protect()(req, res, async () => {
        const updateMicroserviceKnowledgeEndPoint = ResponseDecorator.handleErrors(MicroservicesController.updateMicroserviceKnowledgeEndPoint,
          successCode, errorCodes)
        const responseObject = await updateMicroserviceKnowledgeEndPoint(req)
        const user = req.kauth && req.kauth.grant && req.kauth.grant.access_token ? req.kauth.grant.access_token.content.preferred_username : 'system'
        res
          .status(responseObject.code)
          .send(responseObject.body)
