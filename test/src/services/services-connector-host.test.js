...
describe('services-service connector host', () => {
  ...
  describe('TCP bridge config (no siteId in router microservice payload)', () => {
    ...
    describe('_addTcpConnector()', () => {
      def('fogUuid', () => 'fog-edge')
      def('serviceConfig', () => ({
        type: 'microservice',
        name: 'bridge-svc',
        resource: 'ms-1',
      ...
      def('capture', () => ({ config: null }))

      beforeEach(() => {
        delete process.env.CONTROL_PLANE
        $sandbox.stub(RouterManager, 'findOne').callsFake((where) => {
          if (where.iofogUuid === $fogUuid) {
            return Promise.resolve({ isEdge: true })
          }
          return Promise.resolve(null)
        })
        $sandbox.stub(FogManager, 'findOne').callsFake((where) => {
          if (where.uuid === $fogUuid) {
            return Promise.resolve({ uuid: $fogUuid, name: 'edge-fog' })
          }
          return Promise.resolve(null)
        })
        $sandbox.stub(ApplicationManager, 'findOne').callsFake((where) => {
          if (where.id === 42) {
            return Promise.resolve({ id: 42, name: 'myapp' })
          }
          return Promise.resolve({ id: 10, name: 'system-edge-fog', isSystem: true })
        })
        $sandbox.stub(MicroserviceManager, 'findOne').callsFake((where) => {
          ...
          if (where.name === 'router' && where.applicationId === 10) {
            return Promise.resolve({ uuid: 'router-ms-1', config: '{}' })
          }
          return Promise.resolve(null)
        })
        $sandbox.stub(MicroserviceManager, 'update').callsFake((where, data) => {
          if (data.config) {
            $capture.config = JSON.parse(data.config)
          }
          return Promise.resolve()
        })
      ...
      it('persists connector on the microservice fog router without siteId', async () => {
        await ServicesService._addTcpConnector($serviceConfig, $transaction)

        const connector = $capture.config.bridges.tcpConnectors['bridge-svc-connector']
        expect(connector).to.include({
          name: 'bridge-svc-connector',
          host: 'myapp.worker',
          port: '8080',
          address: 'bridge-svc',
          processId: 'ms-1'
    ...
    describe('_addTcpConnector() default router target', () => {
      def('capture', () => ({ config: null }))
      ...
      beforeEach(() => {
        ...
        $sandbox.stub(RouterManager, 'findOne').callsFake((where) => {
          if (where.isDefault === true) {
            return Promise.resolve({ iofogUuid: 'default-fog' })
          }
          return Promise.resolve(null)
        })
        $sandbox.stub(FogManager, 'findOne').callsFake((where) => {
          if (where.uuid === 'default-fog') {
            return Promise.resolve({ uuid: 'default-fog', name: 'default' })
          }
          return Promise.resolve(null)
        })
        $sandbox.stub(ApplicationManager, 'findOne').resolves({ id: 99, name: 'system-default', isSystem: true })
        $sandbox.stub(MicroserviceManager, 'findOne').callsFake((where) => {
          if (where.name === 'router') {
            return Promise.resolve({ uuid: 'router-default', config: '{}' })
          }
          return Promise.resolve(null)
        })
        $sandbox.stub(MicroserviceManager, 'update').callsFake((where, data) => {
      ...
      it('persists connector on default router for external services without siteId', async () => {
        await ServicesService._addTcpConnector({
          type: 'external',
          name: 'ext-svc',
          resource: 'example.com',
          targetPort: 443
        ...
        const connector = $capture.config.bridges.tcpConnectors['ext-svc-connector']
        expect(connector.host).to.equal('example.com')
        expect(connector).to.not.have.property('siteId')
      })
    })
